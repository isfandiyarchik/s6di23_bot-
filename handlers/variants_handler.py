"""
Пәнлер handler-ы
- Студент пәнди таңлап файл жүклей алады → барлығына жибериледи
- Admin пән қосады/өширеди/атын өзгертеди
"""
import logging
from telebot import types
from database import db_cursor, now_uz, set_user_state, clear_user_state, get_user_state
from handlers.common import (
    is_admin, check_access, check_access_cb,
    back_menu, panler_admin_submenu, main_menu,
    send_to_students, is_already_processed, send_saved_once
)
...
def register(bot):
    ...
    # ── СТУДЕНТ: Пәнлер тізімі ───────────────────────────────
    @bot.message_handler(func=lambda m: m.text == "📖 Пәнлер")
    @ca
    def show_variants_menu(message):
        with db_cursor() as (_, cursor):
            cursor.execute(
                "SELECT subject, COUNT(*) as cnt FROM test_variants "
                "GROUP BY subject ORDER BY subject")
            rows = cursor.fetchall()
        if not rows:
            bot.send_message(message.chat.id, "📭 Еле пән жоқ.",
        ...
        for subj, cnt in rows:
            markup.add(types.InlineKeyboardButton(
                ...
                callback_data=f"var_subj_{subj}"))
        bot.send_message(message.chat.id,
            "📖 <b>Пәнлер</b>\n\nПәнди таңлаңыз:",
            reply_markup=markup)

    # ── СТУДЕНТ: Пән ішіндегі файлдар ───────────────────────
    ...
    def show_variants_by_subject(call):
        ...
        with db_cursor() as (_, cursor):
            cursor.execute(
                "SELECT id,file_name,file_type,date,uploader_username "
                "FROM test_variants WHERE subject=%s ORDER BY date DESC", (subj,))
            rows = cursor.fetchall()
        ...
        # Студент файл жүктей алады
        markup.add(types.InlineKeyboardButton(
            text="📤 Файл жүклеу", callback_data=f"var_upload_{subj}"))
        markup.add(types.InlineKeyboardButton(
            "◀️ Артқа", callback_data="var_back"))

        text = f"📖 <b>{subj}</b>\n\n"
        if rows:
            text += f"📂 {len(rows)} файл бар\n\nФайлды таңлаңыз ямаса тазадан жүклеңиз:"
        else:
            text += "📭 Хазир файл жоқ.\n\n📤 Биринши болып жүклеңиз!"

        bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
            reply_markup=markup, parse_mode="HTML")
        bot.answer_callback_query(call.id)

    # ── СТУДЕНТ: Файл жүктеу батырмасы ──────────────────────
    @bot.callback_query_handler(func=lambda c: c.data.startswith("var_upload_"))
    @cacb
    def var_upload_start(call):
        subj = call.data.replace("var_upload_", "")
        bot.answer_callback_query(call.id)
        set_user_state(call.from_user.id, f"var_upload:{subj}")
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.row("⬅️ Артқа")
        msg = bot.send_message(call.message.chat.id,
            f"📤 <b>{subj}</b> пәнине файл жүклеу\n\n"
            f"Файл, фото ямаса видео жибериңиз.\n"
            f"Барлық студентке жиберилди! 📨",
            reply_markup=markup)

    # ── СТУДЕНТ: Файл қабылдау ───────────────────────────────
    @bot.message_handler(
        content_types=["document", "photo", "video"],
        func=lambda m: get_user_state(m.from_user.id) and
                       get_user_state(m.from_user.id).startswith("var_upload:"))
    @ca
    def handle_var_upload(message):
        if is_already_processed(message.message_id): return
        uid = message.from_user.id
        username = message.from_user.username or f"user{uid}"
        state = get_user_state(uid)
        subj = state.replace("var_upload:", "")
        ...
        if not file_id:
            bot.send_message(message.chat.id, "⚠️ Файл, фото ямаса видео жибериңиз!"); return
        ...
        try:
            with db_cursor() as (conn, cursor):
                cursor.execute(
                    ...
                    "VALUES(%s,%s,%s,%s,%s)",
                    (subj, file_id, file_type, file_name, uid))
                conn.commit()

            clear_user_state(uid)

            # Барлық студентке жібер
            fn = message.from_user.first_name or ""
            send_to_students(bot,
                file_id=file_id, file_type=file_type,
                text=(f"📖 <b>{subj}</b>\n"
                      f"📎 {file_name}\n"
                      f"👤 @{username} ({fn}) жүкледи"),
                exclude_id=uid)

            bot.send_message(message.chat.id,
                f"✅ <b>{file_name}</b> жүкленди!\n"
                f"📖 Пән: {subj}\n"
                f"📨 Барлық студентке жиберилди!",
    ...
    def send_variant_file(call):
        ...
        with db_cursor() as (_, cursor):
            cursor.execute(
                "SELECT file_id,file_type,file_name,subject,uploader_id "
                "FROM test_variants WHERE id=%s", (vid,))
            row = cursor.fetchone()
        if not row:
            bot.answer_callback_query(call.id, "Файл табылмады."); return
        file_id, file_type, file_name, subject, uploader_id = row

        # Кім жүктеген?
        with db_cursor() as (_, cursor):
            cursor.execute("SELECT username,full_name FROM students WHERE id=%s", (uploader_id,))
            ur = cursor.fetchone()
        uploader = f"@{ur[0]}" if ur and ur[0] else (ur[1] if ur else "Admin")
    ...
    def variants_back(call):
        with db_cursor() as (_, cursor):
            ...
            rows = cursor.fetchall()
        if not rows:
            bot.edit_message_text("📭 Пәнлер жоқ.",
    ...
    def panler_admin(message):
        ...
        bot.send_message(message.chat.id, "📖 <b>Пән басқарыу</b>",
    ...
    def handle_variant_subject(message):
        ...
        if not message.text or message.text == "⬅️ Артқа":
            bot.send_message(message.chat.id, "📖 Пән басқарыу",
        ...
        # Бос пән жаса (файлсыз)
        # Пәнді тіркеу үшін бір файл керек немесе жай атын сақтаймыз
        # Бір placeholder жасаймыз
        try:
            with db_cursor() as (conn, cursor):
                cursor.execute(
                    "SELECT COUNT(*) FROM test_variants WHERE subject=%s", (subject,))
                if cursor.fetchone()[0] > 0:
                    bot.send_message(message.chat.id,
                        f"⚠️ <b>{subject}</b> пәни бұрыннан бар!",
                        reply_markup=panler_admin_submenu()); return
                # Пән атын sent_reminders-ке placeholder ретінде сақтаймыз
                # Жоқ, тек хабарлама жіберейін — пән файл жүктегенде пайда болады
            bot.send_message(message.chat.id,
                f"✅ <b>{subject}</b> пәни тайын!\n\n"
                f"📌 Енди студентлер бұл пәнге файл жүктей алады.\n"
                f"📎 Сиз де <b>Пәнге файл қосыу</b> арқалы материал қоса аласыз.",
                reply_markup=panler_admin_submenu())
            # Пәнге бос placeholder қос
            with db_cursor() as (conn, cursor):
                cursor.execute(
                    "INSERT INTO test_variants(subject,file_id,file_type,file_name,uploader_id) "
                    "VALUES(%s,%s,%s,%s,%s)",
                    (subject, "placeholder", "placeholder", f"📖 {subject} пәни", 0))
                conn.commit()
        except Exception as e:
    ...
    def addfile_subject_selected(call):
        ...
        bot.answer_callback_query(call.id)
        set_user_state(call.from_user.id, f"addfile:{subject}")
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.row("⏭ Тайын"); markup.row("⬅️ Артқа")
        msg = bot.send_message(call.message.chat.id,
            f"📖 <b>{subject}</b>\n\n"
            "📤 Файл жибериңиз.\n"
            "<i>Тайын болғанда <b>⏭ Тайын</b> басыңыз.</i>",
            reply_markup=markup)
    ...
    def handle_addfile_loop(message, subject):
        ...
        try:
            ...
            markup.row("⏭ Тайын"); markup.row("⬅️ Артқа")
            msg = bot.send_message(message.chat.id,
                f"✅ <b>{file_name}</b> қосылды!\nКелесини жибериңиз ямаса <b>⏭ Тайын</b>:",
                reply_markup=markup)
            bot.register_next_step_handler(msg, lambda m: handle_addfile_loop(m, subject))
        except Exception as e:
            bot.send_message(message.chat.id, f"❌ DB қатеси: {e}",
    ...
    def edit_variant_subject_start(message):
        ...
        with db_cursor() as (_, cursor):
            ...
            rows = cursor.fetchall()
        if not rows:
            bot.send_message(message.chat.id, "📭 Пәнлер жоқ.",
                reply_markup=panler_admin_submenu()); return
    ...
    def handle_variant_subject_edit_old(message):
        ...
        with db_cursor() as (_, cursor):
            cursor.execute(
                "SELECT COUNT(*) FROM test_variants WHERE subject=%s", (old_subject,))
            cnt = cursor.fetchone()[0]
        if cnt == 0:
            msg = bot.send_message(message.chat.id,
                "⚠️ Бұл атта пән табылмады. Қайта жазыңыз:", reply_markup=back_menu())
            bot.register_next_step_handler(msg, handle_variant_subject_edit_old); return
        msg = bot.send_message(message.chat.id,
            f"✏️ <b>{old_subject}</b> — {cnt} файл\n\nТаза атын жазыңыз:",
            reply_markup=back_menu())
        bot.register_next_step_handler(msg,
            lambda m: handle_variant_subject_edit_save(m, old_subject))
    ...
    def handle_variant_subject_edit_save(message, old_subject):
        ...
        if len(new_subject) < 2 or len(new_subject) > 100:
            msg = bot.send_message(message.chat.id,
                "❌ 2-100 таңба болыуы керек:", reply_markup=back_menu())
            bot.register_next_step_handler(msg,
                lambda m: handle_variant_subject_edit_save(m, old_subject)); return
        try:
            with db_cursor() as (conn, cursor):
                cursor.execute(
                    "UPDATE test_variants SET subject=%s WHERE subject=%s",
    ...
    def delete_variant_start(message):
        ...
        with db_cursor() as (_, cursor):
            ...
            rows = cursor.fetchall()
        if not rows:
            bot.send_message(message.chat.id, "📭 Вариантлар жоқ.",
                reply_markup=panler_admin_submenu()); return
        text = "🗑 <b>Пән/файл өшириу:</b>\n\n"
        for r in rows:
            if r[3] == "placeholder":
                text += f"ID:<code>{r[0]}</code> | {r[1]} | 📁 (бос пән)\n"
            else:
        ...
        try:
            with db_cursor() as (conn, cursor):
                if message.text.strip().lower() == "all":
                    ...
                    cursor.execute(
                        "SELECT file_name,subject FROM test_variants WHERE id=%s", (rid,))
                    row = cursor.fetchone()
                    if not row:
